...
func setupReloadTestServer(t *testing.T) *Server {
...
func TestProxyActionStartFailureAnnotates(t *testing.T) {
    s := setupReloadTestServer(t)
    // override CmdBuilder to fail on Start
    s.FRP.CmdBuilder = func(cfgPath string) *exec.Cmd {
    ...
    err = s.DB.QueryRow(`SELECT annotations FROM proxies WHERE id = ?`, id).Scan(&annotation)
    if err != nil {
        t.Fatal(err)
    }
    if annotation == "" {
        t.Fatalf("expected annotation to contain error message")
    }
}
