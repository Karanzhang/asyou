...
Migrations are in `migrations/`. The server runs them automatically on startup using `CREATE TABLE IF NOT EXISTS`.

New migration: create `migrations/XXXX_description.sql` — they execute in alphabetical order.
